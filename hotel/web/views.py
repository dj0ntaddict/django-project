...
def homepage(request):
    ...
    if request.method =="POST":
        try:
            ...
            if len(room) == 0:
                messages.warning(request,"Disculpa, no hay habitaciones disponibles en este periodo")
    ...
    if request.method =="POST":
        ...
        try:
            if User.objects.all().get(username=user_name):
                messages.warning(request,"Nombre de usuario no disponible")
                return redirect('userloginpage')
        except:
            pass
...
def logoutuser(request):
    if request.method =='GET':
        logout(request)
        messages.success(request,"Desconexion exitosa")
        print("Desconexion exitosa")
        return redirect('homepage')
    else:
        print("Desconexion Fallida")
        return redirect('userloginpage')
...
def add_new_room(request):
    ...
    if request.method == "POST":
        ...
        new_room.save()
        messages.success(request,"Nueva habitacion agregada exitosamente")
    ...
    if request.method =="POST":
        ...
        for each_reservation in Reservation.objects.all().filter(room = room):
            if str(each_reservation.check_in) < str(request.POST['check_in']) and str(each_reservation.check_out) < str(request.POST['check_out']):
                ...
                messages.warning(request,"Esta habitacion no esta disponible para reserva")
                return redirect("homepage")
        ...
        messages.success(request,"Felicidades! Reserva completada")
...
def user_bookings(request):
    ...
    if not bookings:
        messages.warning(request,"No hay reservas encontradas")
